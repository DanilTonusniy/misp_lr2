...
inline std::wstring modAlphaCipher::getValidKey(const std::wstring & ws)
{
    ...
    for (auto & c:tmp) {
        if (!iswalpha(c)) {
            throw cipher_error(std::string("Неверный формат ключа ") + s);
        }
        if (iswlower(c))
            c = towupper(c);
    }
...
inline std::wstring modAlphaCipher::getValidOpenText(const std::wstring & ws)
{
    ...
    if (tmp.empty())
        throw cipher_error("Нет входного текста");
    return tmp;
}
inline std::wstring modAlphaCipher::getValidCipherText(const std::wstring & ws)
{
    if (ws.empty())
        throw cipher_error("Нет выходного текста");
    for (auto c:ws) {
        std::string s = codec.to_bytes(ws);
        if (!iswupper(c))
            throw cipher_error(std::string("Неверный формат выходного текста") + s);
    }
    return ws;
}